...
----
+
... Beachten Sie alle unterstützten link:https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/8/html/configuring_and_managing_high_availability_clusters/assembly_controlling-cluster-behavior-configuring-and-managing-high-availability-clusters["Cluster-Eigenschaft"^] Kann bei Bedarf auch hier angegeben werden. Es ist normalerweise nicht nötig, diese anzupassen, da die BeeGFS HA-Rolle mit einer Reihe gut getesteter ausgeliefert wird link:https://github.com/netappeseries/beegfs/blob/ae038bb2f7617d18a36cc4df8ca9464bd5039b8b/roles/beegfs_ha_7_2/defaults/main.yml#L54["Standardwerte"^].
...
beegfs_ha_fencing_agents:
  fence_redfish:
    ...
    - pcmk_host_list: <HOSTNAME>
      ...
      <<: *redfish
----
... Details zur Konfiguration anderer Zaunmittel finden Sie im link:https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/8/html/configuring_and_managing_high_availability_clusters/assembly_configuring-fencing-configuring-and-managing-high-availability-clusters["Red hat Dokumentation"^].




. Die BeeGFS HA-Rolle kann viele verschiedene Tuning-Parameter anwenden, um die Leistung weiter zu optimieren. Dazu gehören unter anderem die Optimierung der Kernel-Speicherauslastung und die E/A von Blockgeräten. Die Rolle wird mit einem angemessenen Satz von ausgeliefert link:https://github.com/netappeseries/beegfs/blob/release-3.0.1/roles/beegfs_ha_7_2/defaults/main.yml#L106["Standardwerte"^] In Tests mit Block-Nodes der NetApp E-Series werden diese standardmäßig jedoch nicht angewendet, wenn Sie angeben:
+
[source, yaml]
----
...
----
+
.. Geben Sie bei Bedarf auch hier Änderungen an der Standard-Performance-Optimierung an. Siehe die vollständige link:https://github.com/netappeseries/beegfs/blob/release-3.0.1/roles/beegfs_ha_7_2/docs/performance_tuning.md["Parameter für die Performance-Optimierung"^] Dokumentation für weitere Details.
...
. Geben Sie optional die Konfiguration an, die für alle BeeGFS-Dienste im Cluster gelten soll. Es können Standardkonfigurationswerte gefunden werden link:https://github.com/netappeseries/beegfs/blob/release-3.0.1/roles/beegfs_ha_7_2/defaults/main.yml#L159["Hier"^], Und die Konfiguration pro Service ist an anderer Stelle spezifiziert:
+
...
... Weitere Optionen finden Sie in der vollständigen Liste der Standardwerte, die im enthalten sind link:https://github.com/netappeseries/beegfs/blob/release-3.1.0/roles/beegfs_ha_7_3/defaults/main.yml#L32["BeeGFS HA-Rolle"^].
...
. Laden Sie mit Curl oder Ihrem gewünschten Tool die Pakete für die im aufgeführten OpenSM-Versionen herunter link:beegfs-technology-requirements.html["Technologieanforderungen erfüllt"^] Abschnitt von der Mellanox-Website bis zum `<INVENTORY>/packages/` Verzeichnis. Beispiel:
+
[source, bash]
----
