...
[IMPORTANT]
====
Wenn Sie einen bestehenden BeeGFS 7-Cluster auf BeeGFS 8 aktualisieren, befolgen Sie die link:../../administer/clusters-upgrade-beegfs-v8.html["Upgrade auf BeeGFS 8"] Vorgehensweise. Konfigurieren Sie Client-TLS erst, wenn Sie im Rahmen der Aktualisierungsprozedur dazu aufgefordert werden.
...
. Kopieren Sie das Clientzertifikat auf den Clientknoten und benennen Sie es in  `cert.pem` um:
+
[source, console]
----
